...
output1 = gr.Textbox(label="Overview")


with gr.Blocks() as app:
    github_repo = gr.Textbox(label="Github Repo")
    dev_key = gr.Textbox(label="Access Key/Token", placeholder="(Leave blank if repo is public)")
    branch = gr.Radio(
        ["main","master"], label="Initial Branch"
    )
    verbosity = gr.Checkbox(label="Verbose")

    gr.Examples(
        [
            ['https://github.com/IMJONEZZ/NLP', '', 'master', True],
            ['https://github.com/rphovley/storymakerevents','','master',True],
            ['https://github.com/GetPerspectdev/grepit','','main',True]
        ], 
        inputs=[
            github_repo,
            dev_key,
    ...
    start_btn = gr.Button("Detect")
    start_btn.click(
        fn=get_github,
        inputs=[github_repo, dev_key, branch, verbosity],
        outputs=[output1],
        api_name="designpatterndetect"
    )
    output1.render()
    #examples=[['https://github.com/IMJONEZZ/NLP', '', 'master', True],['https://github.com/rphovley/storymakerevents','','master',True],['https://github.com/GetPerspectdev/grepit','','main',True]], 
