...
from service import ScoreProcessor
import threading
...
def health_check():
    return jsonify({"msg": "Perspect Scores API", "status": "all systems go"})
...
def get_design_patterns():
    ...
    threading.Thread(target=processor.enqueue_score, args=(github_repo, score_id, branch)).start()
